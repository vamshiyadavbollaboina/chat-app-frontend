...
const Sidebar = ({ isCollapsed, toggleSidebar }) => {
  ...
  const [deletedIds, setDeletedIds] = useState(() =>
    JSON.parse(localStorage.getItem('deletedSessions') || '[]')
  );
  ...
  const startNewChat = async () => {
    try {
      ...
      const newSession = {
        id: data.sessionId,
        createdAt,
        title: `Chat - ${new Date(createdAt).toLocaleTimeString([], {
          hour: '2-digit',
          minute: '2-digit',
          second: '2-digit'
        })}`
  ...
  return (
    <div className={`flex-shrink-0 h-screen transition-all duration-300 ${
      ...
      border-r dark:border-gray-700 overflow-y-auto fixed md:static z-20`}
    >
      <div className="p-4 flex flex-col h-full">
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            ...
            className="p-1 rounded-full hover:bg-gray-300 dark:hover:bg-gray-700 transition-all duration-200"
          >
            {isCollapsed ? (
        ...
        <div className="mb-4">
          <button
            ...
            className="w-full p-2 rounded-lg bg-gradient-to-r from-green-400 to-green-600 
              hover:from-green-500 hover:to-green-700 text-white font-medium transition-all 
              transform hover:scale-105 shadow-md flex items-center justify-center"
          >
          ...
          {filteredSessions.map(session => (
            <div key={session.id} className="flex items-center justify-between mb-2">
              <NavLink
                to={`/chat/${session.id}`}
                className={({ isActive }) =>
                  `flex-grow block p-3 rounded-lg truncate transition-all duration-200 font-medium ${
                    isActive
              ...
              {!isCollapsed && (
                <button
                  ...
                  className="ml-2 p-2 rounded-lg transition-all hover:bg-red-100 dark:hover:bg-red-700"
        ...
        <div className="mt-4 pt-4 border-t dark:border-gray-700">
          {!isCollapsed && (
            <div className="flex items-center space-x-3 p-2 hover:bg-gray-200 dark:hover:bg-gray-700 
